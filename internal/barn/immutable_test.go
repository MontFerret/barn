...
func TestCheckImmutableAllowsAdditiveVersion(t *testing.T) {
	...
	writeJSON(t, filepath.Join(root, "modules", "montferret", "archive", "versions", "v1.1.0.json"), testVersion("1.1.0", "v1.1.0", strings.Repeat("a", 40)))
...
func TestCheckImmutableRejectsPublishedMutations(t *testing.T) {
	for name, mutate := range map[string]func(*testing.T, string){
		...
		"version deleted": func(t *testing.T, root string) {
			if err := os.Remove(filepath.Join(root, "modules", "montferret", "archive", "versions", "v1.0.0.json")); err != nil {
				t.Fatal(err)
			}
		},
		"repository changed": func(t *testing.T, root string) {
			...
			writeJSON(t, filepath.Join(root, "modules", "montferret", "archive", "manifest.json"), manifest)
		},
